...
- `rack_acabado`: Rack/pallet do produto acabado (numeração automática USI-1001 a USI-1999).
...
## Tabela: `rack_counter`
Controla a numeração automática dos racks de produto acabado.
- `id`: Identificador (serial).
- `tipo`: Tipo do contador (ex.: `usinagem`).
- `proximo_numero`: Próximo número a ser gerado.
- `prefixo`: Prefixo do código (ex.: `USI`).
- `numero_minimo`: Número mínimo do ciclo (ex.: `1001`).
- `numero_maximo`: Número máximo do ciclo (ex.: `1999`).
- `criado_em`: Data/hora de criação.
- `atualizado_em`: Data/hora da última atualização.

### Função: `obter_proximo_rack_usinagem()`
Função PostgreSQL que retorna o próximo rack no formato `USI-####`.
- Incrementa o contador a cada chamada.
- Quando chega em `USI-1999`, retorna para `USI-1001`.
