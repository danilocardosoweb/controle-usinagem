...
const ApontamentosParadas = () => {
  ...
  const { items: paradas, addItem: addParada } = useDatabase('paradas', true)
  ...
  const { items: motivosParada } = useDatabase('motivos_parada', true)
  const { items: tiposParada } = useDatabase('tipos_parada', true)
  ...
  const handleChange = (e) => {
    ...
    if (name === 'motivoParada') {
      const motivo = (motivosParada || []).find(m => String(m.id) === String(value))
      if (motivo) {
        setFormData({
          ...formData,
          motivoParada: value,
          tipoParada: motivo.tipo || formData.tipoParada
        })
        return
      }
    }
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    // Persistir no IndexedDB (store 'paradas')
    const payload = {
  ...
  const paradasRecentes = useMemo(() => {
    const list = (paradas || []).slice().sort((a,b)=>String(b.inicio).localeCompare(String(a.inicio)))
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {paradasRecentes.map((p) => {
                ...
                const tipoNome = p.tipoParada === 'setup' ? 'Setup' :
                  p.tipoParada === 'nao_planejada' ? 'Não Planejada' :
                  p.tipoParada === 'manutencao' ? 'Manutenção' :
                  p.tipoParada === 'planejada' ? 'Planejada' : (p.tipoParada || '-')
                const tipoClass = p.tipoParada === 'setup' ? 'bg-yellow-100 text-yellow-800' :
                  p.tipoParada === 'nao_planejada' ? 'bg-red-100 text-red-800' :
                  p.tipoParada === 'manutencao' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'
                return (
                  <tr key={p.id}>
