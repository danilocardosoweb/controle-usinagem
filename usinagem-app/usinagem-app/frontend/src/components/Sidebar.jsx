...
const Sidebar = ({ isOpen, onToggle, onClose, isMobile }) => {
  ...
  const allMenuItems = [
    ...
    { path: '/expedicao', name: 'Expedição', icon: <FaTruck />, roles: ['admin', 'supervisor', 'operador'] },
            ...
            if (isExternal) {
              ...
              return (
                <a
                  ...
                  className={`flex items-center ${(menuRecolhido && !isMobile) ? 'justify-center' : 'space-x-3'} py-2 px-3 rounded-lg transition-all duration-200 text-blue-100 hover:bg-blue-700 hover:text-white`}
                  title={`${item.name} (Abre em nova guia)`}
                >
