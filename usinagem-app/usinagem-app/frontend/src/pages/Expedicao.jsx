...
import { FaTruck, FaBarcode, FaCheckCircle, FaBox, FaClipboardList, FaHistory, FaPrint, FaPlus, FaSearch, FaFilter, FaDownload, FaTimes, FaCheck, FaExclamationTriangle, FaTrash, FaCubes, FaTruckLoading, FaCalendarAlt, FaUser, FaFolderOpen, FaSync, FaExternalLinkAlt, FaShippingFast, FaUndo } from 'react-icons/fa'
...
export default function Expedicao() {
    ...
    try {
      ...
      racksParaRomaneio.forEach(rack => {
        rack.apontamentos.forEach(apontamento => {
          ...
          // Extrai ferramenta: primeiras letras + 4 dígitos (ex: TG2011 -> TG-2011)
          ...
          // comprimento_acabado_mm pode vir como string do banco — forçar número
          ...
          // lote_externo: campo direto ou primeiro item do array lotes_externos
  ...
  const restaurarRomaneio = async (romaneio) => {
    ...
    try {
      ...
      // 2. Remover baixas de estoque associadas (marcar como estornadas)
  ...
  const exportarParaExcel = async (romaneio) => {
    try {
      ...
      const dados = [
        ...
        [],
        ['ITENS DO ROMANEIO'],
        ['Rack', 'Produto', 'Ferramenta', 'Comp. Acabado (mm)', 'Quantidade', 'Cliente', 'Pedido', 'Lote Externo', 'Status']
      ]
                        ...
                        <div className="flex items-center gap-2 p-3">
                          ...
                          <div className="flex-1 cursor-pointer" onClick={toggleConferido}>
                            ...
                            <p className="text-sm text-gray-500 mt-0.5">
                              {item.produto}{item.ferramenta ? ` · ${item.ferramenta}` : ''}{item.comprimento_acabado_mm ? ` · ${item.comprimento_acabado_mm}mm` : ''} · {item.quantidade} PC
                            </p>
                            {naoEnc && obs && (
