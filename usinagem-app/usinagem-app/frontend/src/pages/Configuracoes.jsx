...
import GerenciadorDocumentosFerramentas from '../components/GerenciadorDocumentosFerramentas'
...
const Configuracoes = () => {
  ...
  // Carteira de Encomendas (Supabase)
  const { items: carteiraEncomendas, addItems: addCarteiraEncomendas, clearItems: clearCarteiraEncomendas } = useSupabase('carteira_encomendas')
  const [arquivo, setArquivo] = useState(null)
  const [arquivoLotes, setArquivoLotes] = useState(null)
  const [arquivoCarteira, setArquivoCarteira] = useState(null)
  ...
  const [mensagemCarteira, setMensagemCarteira] = useState('')
  ...
  const [novoCfg, setNovoCfg] = useState({
    ...
    pecas_por_amarrado: '',
    ripas_por_pallet: '',
    teorico_produtividade_pcs_hora: '',
  ...
  const [filtroExpedicao, setFiltroExpedicao] = useState('')
  const [filtroDocumentos, setFiltroDocumentos] = useState('')

  const ferramentasExpedicaoFiltradas = useMemo(() => {
    const termo = String(filtroExpedicao || '').trim().toLowerCase()
    const itens = Array.isArray(ferramentasCfg) ? ferramentasCfg : []
    if (!termo) return itens

    return itens.filter(cfg => {
      const ferramenta = String(cfg?.ferramenta || '').toLowerCase()
      const comprimento = String(cfg?.comprimento_mm || '').toLowerCase()
      const embalagem = String(cfg?.embalagem || '').toLowerCase()
      return ferramenta.includes(termo) || comprimento.includes(termo) || embalagem.includes(termo)
    })
  }, [ferramentasCfg, filtroExpedicao])

  const ferramentasCfgFiltradas = useMemo(() => {
    const termo = String(filtroDocumentos || '').trim().toLowerCase()
    const itens = Array.isArray(ferramentasCfg) ? ferramentasCfg : []
    if (!termo) return itens
    ...
    return itens.filter(cfg => {
      ...
      const embalagem = String(cfg?.embalagem || '').toLowerCase()
      const comprimento = String(cfg?.comprimento_mm || '').toLowerCase()
      return ferramenta.includes(termo) || embalagem.includes(termo) || comprimento.includes(termo)
    })
  }, [ferramentasCfg, filtroDocumentos])
  ...
  const adicionarCfg = async () => {
    try {
      ...
      const payload = {
        ...
        pecas_por_amarrado: parseInt(novoCfg.pecas_por_amarrado) || 0,
        ripas_por_pallet: isCaixa ? 0 : (parseInt(novoCfg.ripas_por_pallet) || 0),
        teorico_produtividade_pcs_hora: parseFloat(novoCfg.teorico_produtividade_pcs_hora) || 0,
        embalagem: novoCfg.embalagem || 'pallet',
        pcs_por_caixa: isCaixa ? (parseInt(novoCfg.pcs_por_caixa) || 0) : 0
      }
      await addCfg(payload)
      setNovoCfg({ ferramenta: '', peso_linear: '', comprimento_mm: '', pcs_por_pallet: '', pecas_por_amarrado: '', ripas_por_pallet: '', teorico_produtividade_pcs_hora: '', embalagem: 'pallet', pcs_por_caixa: '' })
  ...
  const salvarEdicaoCfg = async () => {
    ...
    try {
      ...
      const payload = {
        id: editandoCfg.id,
        ferramenta: String(editandoCfg.ferramenta).trim(),
        ...
        pecas_por_amarrado: parseInt(editandoCfg.pecas_por_amarrado) || 0,
        ripas_por_pallet: isCaixa ? 0 : (parseInt(editandoCfg.ripas_por_pallet) || 0),
        teorico_produtividade_pcs_hora: parseFloat(editandoCfg.teorico_produtividade_pcs_hora) || 0,
        embalagem: editandoCfg.embalagem || 'pallet',
    ...
    if (tipo === 'termica') {
      try {
        ...
        const tspl = PrintService.gerarEtiquetaTspl({
          larguraEtiquetaMm: Number(impressora.larguraEtiquetaMm || 100),
          alturaEtiquetaMm: Number(impressora.alturaEtiquetaMm || 45),
          gapEtiquetaMm: Number(impressora.gapEtiquetaMm ?? 3),
  ...
  // Importar Carteira de Encomendas via planilha
  const importarCarteiraEncomendas = async () => {
    if (!arquivoCarteira) {
      setErroCarteira('Selecione um arquivo para importar')
      return
    }
    const extensao = arquivoCarteira.name.split('.').pop().toLowerCase()
    if (!['xlsx', 'xls'].includes(extensao)) {
      setErroCarteira('Formato de arquivo não suportado. Por favor, selecione um arquivo Excel (.xlsx ou .xls)')
      return
    }
    ...
    try {
      ...
      reader.onload = async (e) => {
        try {
          const nomeArquivo = arquivoCarteira.name
          const data = new Uint8Array(e.target.result)
          const workbook = XLSX.read(data, { type: 'array' })
          const primeiraPlanilha = workbook.SheetNames[0]
          const planilha = workbook.Sheets[primeiraPlanilha]
          ...
          // Mapear colunas
          colunas.pedido = todasColunas['PEDIDO'] ?? cabecalhos.findIndex(c => norm(c) === 'pedido')
          colunas.cliente = todasColunas['CLIENTE'] ?? cabecalhos.findIndex(c => norm(c) === 'cliente')
          colunas.nr_pedido = todasColunas['NR PEDIDO'] ?? todasColunas['NR.PEDIDO'] ?? cabecalhos.findIndex(c => norm(c).includes('nrpedido') || norm(c).includes('numeropedido'))
          colunas.produto = todasColunas['PRODUTO'] ?? cabecalhos.findIndex(c => norm(c) === 'produto')
          colunas.ferramenta = todasColunas['FERRAMENTA'] ?? cabecalhos.findIndex(c => norm(c) === 'ferramenta')
          colunas.pedido_kg = todasColunas['PEDIDO KG'] ?? todasColunas['PEDIDO.KG'] ?? cabecalhos.findIndex(c => norm(c) === 'pedidokg')
          colunas.pedido_pc = todasColunas['PEDIDO PC'] ?? todasColunas['PEDIDO.PC'] ?? cabecalhos.findIndex(c => norm(c) === 'pedidopc')
          colunas.empenho_kg = todasColunas['EMPENHO KG'] ?? todasColunas['EMPENHO.KG'] ?? cabecalhos.findIndex(c => norm(c) === 'empenhokg')
          colunas.empenho_pc = todasColunas['EMPENHO PC'] ?? todasColunas['EMPENHO.PC'] ?? cabecalhos.findIndex(c => norm(c) === 'empenhopc')
          colunas.produzido_kg = todasColunas['PRODUZIDO KG'] ?? todasColunas['PRODUZIDO.KG'] ?? cabecalhos.findIndex(c => norm(c) === 'produzidokg')
          colunas.produzido_pc = todasColunas['PRODUZIDO PC'] ?? todasColunas['PRODUZIDO.PC'] ?? cabecalhos.findIndex(c => norm(c) === 'produzidopc')
          colunas.embalado_kg = todasColunas['EMBALADO KG'] ?? todasColunas['EMBALADO.KG'] ?? cabecalhos.findIndex(c => norm(c) === 'embaladokg')
          colunas.embalado_pc = todasColunas['EMBALADO PC'] ?? todasColunas['EMBALADO.PC'] ?? cabecalhos.findIndex(c => norm(c) === 'embaladopc')
          colunas.romaneio_kg = todasColunas['ROMANEIO KG'] ?? todasColunas['ROMANEIO.KG'] ?? cabecalhos.findIndex(c => norm(c) === 'romaneiokg')
          colunas.romaneio_pc = todasColunas['ROMANEIO PC'] ?? todasColunas['ROMANEIO.PC'] ?? cabecalhos.findIndex(c => norm(c) === 'romaneiopc')
          
          // Validar colunas essenciais
          const colunasEssenciais = ['pedido', 'cliente']
          const colunasNaoEncontradas = colunasEssenciais.filter(col => colunas[col] === -1)
          if (colunasNaoEncontradas.length > 0) throw new Error(`Colunas obrigatórias não encontradas: ${colunasNaoEncontradas.join(', ')}`)
          ...
          for (let i = 1; i < dadosPlanilha.length; i++) {
            ...
            const pedido = String(linha[colunas.pedido] || '').trim()
            const cliente = String(linha[colunas.cliente] || '').trim()
            const nr_pedido = colunas.nr_pedido !== -1 ? String(linha[colunas.nr_pedido] || '').trim() : ''
            const produto = colunas.produto !== -1 ? String(linha[colunas.produto] || '').trim() : ''
            const ferramenta = colunas.ferramenta !== -1 ? String(linha[colunas.ferramenta] || '').trim() : ''
            const pedido_kg = colunas.pedido_kg !== -1 ? toNumberBR(linha[colunas.pedido_kg]) : 0
            const pedido_pc = colunas.pedido_pc !== -1 ? toNumberBR(linha[colunas.pedido_pc]) : 0
            const empenho_kg = colunas.empenho_kg !== -1 ? toNumberBR(linha[colunas.empenho_kg]) : 0
            const empenho_pc = colunas.empenho_pc !== -1 ? toNumberBR(linha[colunas.empenho_pc]) : 0
            const produzido_kg = colunas.produzido_kg !== -1 ? toNumberBR(linha[colunas.produzido_kg]) : 0
            const produzido_pc = colunas.produzido_pc !== -1 ? toNumberBR(linha[colunas.produzido_pc]) : 0
            const embalado_kg = colunas.embalado_kg !== -1 ? toNumberBR(linha[colunas.embalado_kg]) : 0
            const embalado_pc = colunas.embalado_pc !== -1 ? toNumberBR(linha[colunas.embalado_pc]) : 0
            const romaneio_kg = colunas.romaneio_kg !== -1 ? toNumberBR(linha[colunas.romaneio_kg]) : 0
            const romaneio_pc = colunas.romaneio_pc !== -1 ? toNumberBR(linha[colunas.romaneio_pc]) : 0
            
            novasEncomendas.push({
              pedido,
              cliente,
              nr_pedido: nr_pedido || null,
          ...
          if (novasEncomendas.length === 0) throw new Error('Nenhuma encomenda válida encontrada na planilha')
          
          // Inserir em lotes menores
          const BATCH_SIZE = 50
          let totalInseridos = 0
          
          for (let i = 0; i < novasEncomendas.length; i += BATCH_SIZE) {
            const lote = novasEncomendas.slice(i, i + BATCH_SIZE)
            await addCarteiraEncomendas(lote)
            totalInseridos += lote.length
            console.log(`Inseridas ${totalInseridos}/${novasEncomendas.length} encomendas...`)
          }
          
          setMensagemCarteira(`Arquivo ${nomeArquivo} importado com sucesso! ${novasEncomendas.length} encomendas foram processadas.`)
          setCarregandoCarteira(false)
          setArquivoCarteira(null)
          const inputArquivo = document.querySelector('input[type="file"][id="arquivo-carteira"]')
          if (inputArquivo) inputArquivo.value = ''
        } catch (err) {
          setErroCarteira('Erro ao processar o arquivo: ' + err.message)
          setCarregandoCarteira(false)
        }
      }
      reader.onerror = () => { setErroCarteira('Erro ao ler o arquivo'); setCarregandoCarteira(false) }
      reader.readAsArrayBuffer(arquivoCarteira)
    } catch (error) {
      setErroCarteira('Erro ao importar arquivo: ' + (error.response?.data?.detail || error.message))
      setCarregandoCarteira(false)
    }
  }
    ...
    <div className="space-y-6">
      ...
      <div className="border-b border-gray-200">
        <nav className="-mb-px flex flex-wrap sm:flex-nowrap sm:space-x-4 lg:space-x-8 overflow-x-auto pb-1 gap-2 sm:gap-0">
          ...
          <button
            className={`py-3 px-2 sm:px-3 lg:px-4 border-b-2 font-medium text-xs sm:text-sm whitespace-nowrap transition-colors ${abaAtiva === 'documentos-ferramentas'
              ? 'border-blue-500 text-blue-600'
              : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
              }`}
            onClick={() => setAbaAtiva('documentos-ferramentas')}
          >
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="bg-gray-50 p-3 rounded-lg mb-6">
              <h3 className="text-sm font-medium text-gray-700 mb-2">{modoEdicaoCfg ? 'Editar' : 'Adicionar'}</h3>
              <div className={`grid grid-cols-2 ${((modoEdicaoCfg ? editandoCfg?.embalagem : novoCfg.embalagem) === 'caixa') ? 'md:grid-cols-9' : 'md:grid-cols-8'} gap-2`}>
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-0.5">Ferramenta *</label>
                  <input type="text" className="input-field text-sm py-1 px-2" value={modoEdicaoCfg ? editandoCfg.ferramenta : novoCfg.ferramenta}
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-0.5">Peso Linear (kg)</label>
                  <input type="number" step="0.001" className="input-field text-sm py-1 px-2" value={modoEdicaoCfg ? editandoCfg.peso_linear : novoCfg.peso_linear}
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-0.5">Comprimento (mm)</label>
                  <input type="number" className="input-field text-sm py-1 px-2" value={modoEdicaoCfg ? editandoCfg.comprimento_mm : novoCfg.comprimento_mm}
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-0.5">Pcs por Pallet</label>
                  <input type="number" className="input-field text-sm py-1 px-2" value={modoEdicaoCfg ? editandoCfg.pcs_por_pallet : novoCfg.pcs_por_pallet}
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-0.5">Pcs/Amarrado</label>
                  <input type="number" className="input-field text-sm py-1 px-2" value={modoEdicaoCfg ? (editandoCfg.pecas_por_amarrado ?? '') : novoCfg.pecas_por_amarrado}
                    onChange={(e) => modoEdicaoCfg ? setEditandoCfg({ ...editandoCfg, pecas_por_amarrado: e.target.value }) : setNovoCfg({ ...novoCfg, pecas_por_amarrado: e.target.value })} />
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-0.5">Ripas por Pallet</label>
                  <input type="number" className="input-field text-sm py-1 px-2" value={modoEdicaoCfg ? editandoCfg.ripas_por_pallet : novoCfg.ripas_por_pallet}
                    disabled={modoEdicaoCfg ? (editandoCfg.embalagem === 'caixa') : (novoCfg.embalagem === 'caixa')}
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-0.5">Teórico (pcs/h)</label>
                  <input type="number" step="0.01" className="input-field text-sm py-1 px-2" value={modoEdicaoCfg ? (editandoCfg.teorico_produtividade_pcs_hora ?? '') : novoCfg.teorico_produtividade_pcs_hora}
                    onChange={(e) => modoEdicaoCfg ? setEditandoCfg({ ...editandoCfg, teorico_produtividade_pcs_hora: e.target.value }) : setNovoCfg({ ...novoCfg, teorico_produtividade_pcs_hora: e.target.value })} />
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-0.5">Embalagem</label>
                  <select className="input-field text-sm py-1 px-2" value={modoEdicaoCfg ? editandoCfg.embalagem : novoCfg.embalagem}
                    onChange={(e) => {
                ...
                {(modoEdicaoCfg ? editandoCfg?.embalagem === 'caixa' : novoCfg.embalagem === 'caixa') && (
                  <div>
                    <label className="block text-xs font-medium text-gray-700 mb-0.5">Pcs por Caixa</label>
                    <input type="number" className="input-field text-sm py-1 px-2" value={modoEdicaoCfg ? editandoCfg.pcs_por_caixa : novoCfg.pcs_por_caixa}
                ...
                {!modoEdicaoCfg && (
                  <button type="button" className="btn-primary text-sm py-1 px-3 h-fit self-end" onClick={adicionarCfg}><FaSave className="mr-1" /> Adicionar</button>
                )}
              </div>
              {modoEdicaoCfg && (
              <div className="mt-2 flex justify-end space-x-2">
                <button type="button" className="btn-outline text-sm py-1 px-3" onClick={cancelarEdicaoCfg}><FaTimes className="mr-1" /> Cancelar</button>
                <button type="button" className="btn-primary text-sm py-1 px-3" onClick={salvarEdicaoCfg}><FaSave className="mr-1" /> Salvar</button>
              </div>
              )}
            </div>
            ...
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-4">
              <div>
                <h3 className="text-sm font-medium text-gray-700 mb-1">Cadastros</h3>
                <p className="text-xs text-gray-500">Filtre por matriz, comprimento ou embalagem para revisar e editar mais rápido.</p>
              </div>
              <div className="w-full md:w-80">
                <label className="block text-xs font-medium text-gray-600 mb-1">Filtrar matriz/comprimento</label>
                <input
                  type="text"
                  value={filtroExpedicao}
                  onChange={(e) => setFiltroExpedicao(e.target.value)}
                  placeholder="Ex.: TPS-046 ou 2400"
            ...
            <div className="mb-3 text-xs text-gray-500">
              {ferramentasExpedicaoFiltradas.length} item(ns) exibido(s) de {(ferramentasCfg || []).length}
            </div>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Pcs/Amarrado</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Ripas/Pallet</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Teórico (pcs/h)</th>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {ferramentasExpedicaoFiltradas.map(cfg => (
                    <tr key={cfg.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-blue-700">{cfg.pecas_por_amarrado || '-'}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{cfg.ripas_por_pallet}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{cfg.teorico_produtividade_pcs_hora}</td>
                  ...
                  {(ferramentasCfg || []).length === 0 && (
                    <tr>
                      <td className="px-6 py-6 text-center text-gray-500" colSpan="10">Nenhum cadastro</td>
                    </tr>
                  )}
                  {(ferramentasCfg || []).length > 0 && ferramentasExpedicaoFiltradas.length === 0 && (
                    <tr>
                      <td className="px-6 py-6 text-center text-gray-500" colSpan="10">Nenhum cadastro encontrado para o filtro informado</td>
                    </tr>
                  )}
                </tbody>
              </table>
      ...
      {/* Aba: Documentos de Ferramentas */}
      {abaAtiva === 'documentos-ferramentas' && (
        <div className="space-y-6">
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-6">
              <div>
                <h2 className="text-lg font-semibold text-gray-700 mb-1">Documentos por Ferramenta</h2>
                <p className="text-sm text-gray-600">
                  Itens refletidos da aba Expedição. Use o filtro para localizar rapidamente uma matriz.
                </p>
              </div>

              <div className="w-full md:w-80">
                <label className="block text-xs font-medium text-gray-600 mb-1">Filtrar matriz/ferramenta</label>
                <input
                  type="text"
                  value={filtroDocumentos}
                  onChange={(e) => setFiltroDocumentos(e.target.value)}
                  placeholder="Ex.: TPS-045"
                  className="input-field text-sm"
                />
              </div>
            </div>
            ...
            <div className="mb-4 text-xs text-gray-500">
              {ferramentasCfgFiltradas.length} item(ns) exibido(s) de {(ferramentasCfg || []).length}
            </div>
            ...
            ) : ferramentasCfgFiltradas.length === 0 ? (
              <div className="bg-gray-50 rounded-lg p-8 text-center text-gray-500">
                Nenhum item encontrado para o filtro informado.
              </div>
            ) : (
              <div className="space-y-3">
                {ferramentasCfgFiltradas.map(cfg => (
                  <div key={cfg.id} className="border border-gray-200 rounded-lg p-3 bg-white">
                    <GerenciadorDocumentosFerramentas
                      configId={cfg.id}
                      documentoKey={`${cfg.ferramenta}__${cfg.comprimento_mm}`}
                      ferramentaNome={cfg.ferramenta}
                      comprimentoMm={cfg.comprimento_mm}
                    />
                  </div>
                ))}
              </div>
      ...
      {abaAtiva === 'dados' && (
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-lg shadow p-4 form-compact">
            <h2 className="text-lg font-semibold text-gray-700 mb-3">Dados • Carteira de Encomendas</h2>
            <div className="flex items-center gap-2 mb-2">
              <input
                id="arquivo-carteira"
                type="file"
                accept=".xlsx,.xls"
                onChange={(e) => setArquivoCarteira(e.target.files[0])}
                className="block w-full text-xs text-gray-500 file:mr-3 file:py-2 file:px-3 file:rounded-md file:border-0 file:text-xs file:font-semibold file:bg-primary-50 file:text-primary-700 hover:file:bg-primary-100"
              />
              <button
                type="button"
                className="btn-primary text-xs px-3 py-2 flex items-center"
                onClick={importarCarteiraEncomendas}
                disabled={carregandoCarteira || !arquivoCarteira}
              >
                <FaFileUpload className="mr-2" /> {carregandoCarteira ? 'Importando...' : 'Importar Carteira'}
              </button>
            </div>
            {(erroCarteira || mensagemCarteira) && (
              <div className="text-xs mb-1">
                {erroCarteira && <div className="text-red-600">{erroCarteira}</div>}
                {mensagemCarteira && <div className="text-green-600">{mensagemCarteira}</div>}
              </div>
            )}
            <div className="text-xs text-gray-500">
              <p>Total de encomendas: {carteiraEncomendas.length}</p>
              <p>Suporta Excel (.xlsx, .xls)</p>
            </div>
          </div>
        </div>
