...
/**
 * Modal de Aprovação de Inspeção por Lote
 * 
 * Permite aprovar lotes (total ou parcial) da inspeção para embalagem.
 * 
 * @param {Object} props - Propriedades do componente
 * @param {boolean} props.open - Se o modal está aberto
 * @param {Object} props.pedido - Dados do pedido
 * @param {Array} props.itens - Lista de lotes disponíveis para aprovação
 * @param {boolean} props.saving - Se está processando
 * @param {string} props.error - Mensagem de erro
 * @param {Function} props.onClose - Handler de fechar
 * @param {Function} props.onConfirm - Handler de confirmar aprovação
 * @param {Function} props.onSetMover - Handler para alterar quantidade a mover
 * @param {Function} props.onAprovarTudo - Handler para preencher tudo
 * 
 * @created 18/11/2024
 * @author Cascade AI
 */
                ...
                {itens.length === 0 ? (
                  <tr>
                    <td className="py-2 pr-3 text-sm text-gray-500" colSpan={3}>
                      Nenhum lote disponível para aprovação.
                    </td>
                  </tr>
                ) : (
                  itens.map((it) => (
                    <tr key={`aprov-${pedido?.id}-${it.lote}`}>
