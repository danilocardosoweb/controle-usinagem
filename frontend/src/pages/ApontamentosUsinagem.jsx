...
import { FaSearch, FaFilePdf, FaBroom, FaListUl, FaPlus, FaCopy, FaStar, FaWrench, FaFileAlt, FaFileExcel } from 'react-icons/fa'
...
import VisualizadorDocumentosFerramentas from '../components/VisualizadorDocumentosFerramentas'
...
const ApontamentosUsinagem = ({ tituloPagina = 'Apontamentos de Usinagem', subtituloForm = 'Novo Apontamento', modo = 'usinagem' }) => {
  ...
  const [filtroTabelaApont, setFiltroTabelaApont] = useState('')
  ...
  // Modal de visualização de documentos da ferramenta
  const [documentosAberto, setDocumentosAberto] = useState(false)
  const [ferramentaSelecionadaDocumentos, setFerramentaSelecionadaDocumentos] = useState('')
  ...
  const toggleAmarradoSelecionado = (amarrado) => {
    const existe = amarradosSelecionadosRack.some(a => a.codigo === amarrado.codigo && a.lote === amarrado.lote)
    
    // Se está desmarcando, não precisa validar
    if (existe) {
      setAmarradosSelecionadosRack(prev => 
        prev.filter(a => !(a.codigo === amarrado.codigo && a.lote === amarrado.lote))
      )
      return
    }
    
    // Se está marcando, validar comprimento do lote vs Perfil Longo do item
    const loteObj = lotesEncontrados.find(l => l.lote === amarrado.lote)
    if (loteObj) {
      const comprimentoLote = extrairComprimentoLote(loteObj.produto)
      const comprimentoPerfilLongo = extrairComprimentoPerfilLongo(formData.perfilLongo)
      
      // Se ambos têm comprimento e são diferentes, perguntar ao usuário
      if (comprimentoLote && comprimentoPerfilLongo && comprimentoLote !== comprimentoPerfilLongo) {
        const mensagem = `⚠️ ATENÇÃO: Comprimento do lote (${comprimentoLote}mm) é diferente do Perfil Longo do item (${comprimentoPerfilLongo}mm).\n\nVocê tem certeza que deseja usar este amarrado? Isso pode ser útil para aproveitar material em outro item.`
    ...
    setAmarradosSelecionadosRack(prev => [...prev, amarrado])
  }
  ...
  const selecionarTodosAmarradosDoLote = (lote) => {
    ...
    // Validar comprimento do lote vs Perfil Longo do item
    const comprimentoLote = extrairComprimentoLote(loteObj.produto)
    const comprimentoPerfilLongo = extrairComprimentoPerfilLongo(formData.perfilLongo)
    
    // Se ambos têm comprimento e são diferentes, perguntar ao usuário
    if (comprimentoLote && comprimentoPerfilLongo && comprimentoLote !== comprimentoPerfilLongo) {
      const mensagem = `⚠️ ATENÇÃO: Comprimento do lote (${comprimentoLote}mm) é diferente do Perfil Longo do item (${comprimentoPerfilLongo}mm).\n\nVocê tem certeza que deseja usar este lote? Isso pode ser útil para aproveitar material em outro item.`
      if (!window.confirm(mensagem)) {
        return
      }
    }
  ...
  // Extrai o comprimento do Perfil Longo (ex: "TR003217680NANV" -> "6000 mm" ou "6000mm")
  const extrairComprimentoPerfilLongo = (perfilLongo) => {
    if (!perfilLongo) return null
    const str = String(perfilLongo).trim()
    // Procura por padrão "XXXX mm" ou "XXXXmm" no final da string
    const match = str.match(/(\d{4})\s*mm$/i)
    if (match) {
      return match[1]
    }
    return null
  ...
  // Extrai o comprimento do lote (do produto do lote)
  const extrairComprimentoLote = (produtoLote) => {
    if (!produtoLote) return null
    const comp = extrairComprimentoAcabado(produtoLote)
    if (comp) {
      // Remove "mm" e retorna apenas o número
      return comp.replace(/\D/g, '')
    }
    return null
  }
  ...
  const linhasTabelaApontamentos = useMemo(() => {
    return apontamentosDaOrdem.map((a, idx) => {
      ...
      const pedidoCliente = String(a.pedido_cliente || a.pedidoCliente || '').trim()
      const codigoProdutoCliente = String(a.codigo_produto_cliente || a.codigoProdutoCliente || '').trim()
      const maquina = String(a.maquina || '').trim()
      const operador = String(a.operador || '').trim()
      const produto = String(a.produto || '').trim()
      const rackPallet = String(a.rack_ou_pallet || a.rackOuPallet || '').trim()
      const lote = String(a.lote || '').trim()
      const loteExterno = String(a.lote_externo || a.loteExterno || '').trim()
      const romaneio = String(a.romaneio_numero || a.romaneioNumero || '').trim()
      const nroOp = String(a.nro_op || a.nroOp || '').trim()
      const unidade = String(a.unidade || '').trim()
      const observacoes = String(a.observacoes || '').trim()
      const etapa = String(a.etapa_embalagem || '').trim()
      const statusMigracao = ordem && quantidade > 0 ? 'LANÇADO NO APP' : 'PENDENTE'
  ...
  const linhasTabelaApontamentosFiltradas = useMemo(() => {
    const termo = String(filtroTabelaApont || '').trim().toLowerCase()
    if (!termo) return linhasTabelaApontamentos

    return linhasTabelaApontamentos.filter((linha) => {
      return [
        linha.dataInicio,
        linha.dataFim,
        linha.ordem,
  ...
  }, [linhasTabelaApontamentos, filtroTabelaApont])

  const exportarTabelaApontamentosExcel = () => {
    if (!linhasTabelaApontamentosFiltradas.length) {
      alert('Não há apontamentos para exportar.')
      return
    }

    const rows = linhasTabelaApontamentosFiltradas.map((linha) => ({
      'Data Início': linha.dataInicio,
      'Data Fim': linha.dataFim,
      'Pedido/Seq': linha.ordem,
      'Máquina': linha.maquina,
      ...
      'Lote': linha.lote,
      'Lote Externo': linha.loteExterno,
    ...
    try {
      ...
      worksheet['!cols'] = headers.map((header) => {
        const maxWidth = rows.reduce((acc, row) => Math.max(acc, String(row[header] || '').length), header.length)
        return { wch: Math.min(maxWidth + 2, 40) }
      })
      XLSX.utils.book_append_sheet(workbook, worksheet, sanitizeSheetName(`Apontamentos_${formData.ordemTrabalho || 'Consulta'}`))
      XLSX.writeFile(workbook, `apontamentos_${formData.ordemTrabalho || 'consulta'}_${new Date().toISOString().slice(0, 10)}.xlsx`, { compression: true })
    } catch (error) {
      console.error('Erro ao exportar apontamentos para Excel:', error)
      alert('Erro ao exportar para Excel: ' + (error?.message || 'erro desconhecido'))
    }
  }
  ...
  const linhasTabelaDiaria = useMemo(() => {
    ...
    return (apontamentosDB || []).filter((a) => {
      ...
      const inicio = String(a.inicio || '').slice(0, 10)
      const fim = String(a.fim || '').slice(0, 10)
  ...
  }, [apontamentosDB, dataTabelaDiaria])

  const linhasTabelaDiariaFiltradas = useMemo(() => {
    const termo = String(filtroTabelaDiaria || '').trim().toLowerCase()
    if (!termo) return linhasTabelaDiaria
    return linhasTabelaDiaria.filter((linha) => {
      return [
        linha.dataInicio,
        linha.dataFim,
        linha.ordem,
  ...
  }, [linhasTabelaDiaria, filtroTabelaDiaria])
  ...
  const exportarTabelaDiariaExcel = () => {
    ...
    try {
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, sanitizeSheetName(`Apontamentos_Dia_${dataTabelaDiaria || 'Consulta'}`))
      XLSX.writeFile(workbook, `apontamentos_dia_${dataTabelaDiaria || 'consulta'}.xlsx`, { compression: true })
    } catch (error) {
      console.error('Erro ao exportar apontamentos do dia para Excel:', error)
      alert('Erro ao exportar para Excel: ' + (error?.message || 'erro desconhecido'))
    }
  }
    ...
    <div className="space-y-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                title="Abrir tabela de apontamentos deste pedido"
                onClick={() => setListarApontAberto(true)}
                aria-label="Abrir tabela de apontamentos do pedido"
              >
                <FaListUl />
              </button>
            </div>
              ...
              <div className="flex gap-2">
                ...
                {formData.maquina && (
                  <button
                    type="button"
                    onClick={() => {
                      setFerramentaSelecionadaDocumentos(formData.maquina)
                      setDocumentosAberto(true)
                    }}
                    title="Ver documentos da ferramenta"
                    className="px-3 py-2 bg-blue-500 text-white rounded hover:bg-blue-600 text-sm flex items-center gap-1 whitespace-nowrap"
                  >
      ...
      {listarApontAberto && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-lg shadow-lg w-[96vw] max-w-[1500px] p-4 form-compact">
            <div className="flex items-center justify-between gap-3 mb-3">
              <div>
                <h3 className="text-base font-semibold text-gray-800">Tabela de Apontamentos do Pedido {formData.ordemTrabalho}</h3>
                <p className="text-xs text-gray-500 mt-1">Consulta em formato semelhante à planilha para apoiar conferência e migração.</p>
              </div>
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  className="btn-secondary flex items-center gap-2"
                  onClick={exportarTabelaApontamentosExcel}
            ...
            <div className="mb-3">
              <input
                type="text"
                className="input-field input-field-sm"
                placeholder="Filtrar por pedido, cliente, máquina, operador, lote, romaneio, status..."
                value={filtroTabelaApont}
                onChange={(e) => setFiltroTabelaApont(e.target.value)}
              />
            </div>
            <div className="mb-3 flex items-center justify-between text-xs text-gray-600">
              <span>Total de linhas: {linhasTabelaApontamentosFiltradas.length}</span>
              <span>Status atual representa conferência no app. Se você me indicar o campo do outro sistema, eu ligo essa coluna ao valor real.</span>
            </div>
              ...
              <table className="min-w-full text-sm">
                <thead className="bg-gray-50 sticky top-0 z-10">
                  <tr>
                    ...
                    <th className="text-left px-3 py-2 whitespace-nowrap">Máquina</th>
                    ...
                    <th className="text-left px-3 py-2 whitespace-nowrap">Un.</th>
                    ...
                    <th className="text-left px-3 py-2 whitespace-nowrap">Lote Externo</th>
                    <th className="text-left px-3 py-2 whitespace-nowrap">Romaneio</th>
                    <th className="text-left px-3 py-2 whitespace-nowrap">Nº OP</th>
                    {modo === 'embalagem' && <th className="text-left px-3 py-2 whitespace-nowrap">Etapa</th>}
                ...
                <tbody>
                  {linhasTabelaApontamentosFiltradas.map((linha, idx) => {
                    const apontamentoOriginal = apontamentosDaOrdem[idx] || apontamentosDaOrdem.find((a) => String(a.id || '') === String(linha.id || ''))
                    return (
                    <tr key={linha.id} className="border-t hover:bg-gray-50 align-top">
                      ...
                      <td className="px-3 py-2 whitespace-nowrap">{linha.maquina}</td>
                      ...
                      <td className="px-3 py-2 whitespace-nowrap">{linha.unidade}</td>
                      <td className="px-3 py-2 whitespace-nowrap">{linha.rackPallet}</td>
                      <td className="px-3 py-2 whitespace-nowrap">{linha.lote}</td>
                      <td className="px-3 py-2 whitespace-nowrap">{linha.loteExterno}</td>
                      <td className="px-3 py-2 whitespace-nowrap">{linha.romaneio}</td>
                      <td className="px-3 py-2 whitespace-nowrap">{linha.nroOp}</td>
                      {modo === 'embalagem' && (
                        <td className="px-3 py-2 whitespace-nowrap">
                          {linha.etapa || 'embalagem'}
                        </td>
                      )}
                      <td className="px-3 py-2 min-w-[220px]">{linha.observacoes}</td>
                      <td className="px-3 py-2 whitespace-nowrap">
                        <span className={`px-2 py-1 rounded text-xs font-semibold ${linha.statusMigracao === 'LANÇADO NO APP' ? 'bg-blue-100 text-blue-700' : 'bg-amber-100 text-amber-700'}`}>
                          {linha.statusMigracao}
                        </span>
                      </td>
                      {isAdmin() && (
                        <td className="px-3 py-2 text-center">
                          <button
                            onClick={() => apontamentoOriginal && setApontamentoParaCorrigir(apontamentoOriginal)}
                  ...
                  {linhasTabelaApontamentosFiltradas.length === 0 && (
                    <tr>
                      <td colSpan={String((isAdmin() ? 20 : 19) + (modo === 'embalagem' ? 1 : 0))} className="px-3 py-6 text-center text-gray-500">Nenhum apontamento encontrado para este pedido</td>
                    </tr>
                  )}
                </tbody>
              </table>
      ...
      {tabelaDiariaAberta && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-lg shadow-lg w-[98vw] max-w-[1650px] p-4 form-compact">
            <div className="flex items-center justify-between gap-3 mb-3">
              <div>
                <h3 className="text-base font-semibold text-gray-800">Tabela Diária de Apontamentos</h3>
                <p className="text-xs text-gray-500 mt-1">Visão diária em formato semelhante à planilha para conferência e migração.</p>
              </div>
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  className="btn-secondary flex items-center gap-2"
                  onClick={exportarTabelaDiariaExcel}
            ...
            <div className="grid grid-cols-1 md:grid-cols-[220px_1fr] gap-3 mb-3">
              <div>
                ...
                <input
                  type="date"
                  className="input-field input-field-sm"
                  value={dataTabelaDiaria}
                  onChange={(e) => setDataTabelaDiaria(e.target.value)}
                />
              </div>
              <div>
                ...
                <input
                  type="text"
                  className="input-field input-field-sm"
                  placeholder="Filtrar por data, pedido, cliente, máquina, operador, lote, romaneio, status..."
                  value={filtroTabelaDiaria}
                  onChange={(e) => setFiltroTabelaDiaria(e.target.value)}
                />
              </div>
            </div>
            <div className="mb-3 flex items-center justify-between text-xs text-gray-600">
              <span>Total de linhas: {linhasTabelaDiariaFiltradas.length}</span>
              <span>Esta tabela é independente da consulta do item e mostra os apontamentos do dia selecionado.</span>
            </div>
            <div className="max-h-[72vh] overflow-auto border rounded">
              <table className="min-w-full text-sm">
                <thead className="bg-gray-50 sticky top-0 z-10">
                  <tr>
                    <th className="text-left px-3 py-2 whitespace-nowrap">Data Início</th>
                ...
                <tbody>
                  {linhasTabelaDiariaFiltradas.map((linha) => (
                    <tr key={linha.id} className="border-t hover:bg-gray-50 align-top">
                      ...
                      <td className="px-3 py-2 whitespace-nowrap">{linha.nroOp}</td>
                      {modo === 'embalagem' && (
                        <td className="px-3 py-2 whitespace-nowrap">{linha.etapa || 'embalagem'}</td>
                      )}
                      <td className="px-3 py-2 min-w-[220px]">{linha.observacoes}</td>
                      <td className="px-3 py-2 whitespace-nowrap">
                        <span className={`px-2 py-1 rounded text-xs font-semibold ${linha.statusMigracao === 'LANÇADO NO APP' ? 'bg-blue-100 text-blue-700' : 'bg-amber-100 text-amber-700'}`}>
                      ...
                      {isAdmin() && (
                        <td className="px-3 py-2 text-center">
                          <button
                            onClick={() => linha.original && setApontamentoParaCorrigir(linha.original)}
                            className="text-blue-600 hover:text-blue-800 hover:underline text-xs font-medium"
                            title="Corrigir apontamento"
                          >
                            <FaWrench className="inline mr-1" />
                  ...
                  {linhasTabelaDiariaFiltradas.length === 0 && (
                    <tr>
                      <td colSpan={String((isAdmin() ? 20 : 19) + (modo === 'embalagem' ? 1 : 0))} className="px-3 py-6 text-center text-gray-500">Nenhum apontamento encontrado para a data selecionada</td>
                    </tr>
                  )}
                </tbody>
              </table>
      ...
      {/* Modal: Visualizar Documentos da Ferramenta */}
      <VisualizadorDocumentosFerramentas
        ferramentaId={ferramentaSelecionadaDocumentos}
        ferramentaNome={ferramentaSelecionadaDocumentos}
        isOpen={documentosAberto}
        onClose={() => setDocumentosAberto(false)}
      />
    </div>
  )
}
