...
const Relatorios = () => {
  ...
  const toISODate = (val) => {
    if (!val) return null
    // Se já for uma data no formato YYYY-MM-DD (ex: vinda do input date), retorna direto
    if (typeof val === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(val)) return val.slice(0, 10)
    try {
      const d = new Date(val)
      if (isNaN(d.getTime())) return null
      // Obtém os valores da data baseada no fuso local e não no UTC
  ...
  const apontamentosFiltrados = useMemo(() => {
    ...
    const hoje = toISODate(getTodayDateInput())
    return (apontamentos || []).filter(a => {
      const dd = toISODate(a.inicio)
      if (!dd || dd !== hoje) return false
      if (filtros.maquina) {
